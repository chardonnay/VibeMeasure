...
final class UsageWindowTests: XCTestCase {
    ...
    func testMiniMaxShowsUnreportedAudioAndMusicQuotas() throws {
        let json = """
        {
          "model_remains": [
            {
        ...
        XCTAssertTrue(windows.contains { $0.name == "Audio 5-Hour" })
        XCTAssertTrue(windows.contains { $0.name == "Audio 1 Week" })
        XCTAssertTrue(windows.contains { $0.name == "Music 5-Hour" })
        XCTAssertTrue(windows.contains { $0.name == "Music 1 Week" })
        XCTAssertTrue(
            windows
                .filter { $0.name.hasPrefix("Audio") || $0.name.hasPrefix("Music") }
                .allSatisfy {
                    $0.percent == 0
                        && $0.resetText == "Not reported by mmx quota show; no verified usage or available quota"
                }
        )
    }
}
